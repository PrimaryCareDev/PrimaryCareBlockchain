...
const DoctorViewPatientPopup = (props) => {
    const {data} = props
    ...
    function BottomSection() {
        if (relationship === null || relationship.status === requestStatus.REJECTED || relationship.status === requestStatus.DELETED) {
            return <div
                ...
                <span>Do you want to request access to this patient's data?</span>
                    ...
                    confirmRemoval
                        ...
                        <>
                            <div
                                ...
                                <span>Are you sure you want to remove this patient?</span>
                        ...
                        <>
                            <div
                                ...
                                <span>You are connected with this patient.{` `}
            ...
            return <>
                <div
                    ...
                    <ExclamationCircleIcon className="h-12"/>
                    <span>Your request has been sent. Please wait for the patient to accept your request.</span>
                    ...
                    confirmRejection
                        ...
                        <>
                            <div
                                ...
                                <span>Are you sure you want to reject this patient's invitation?</span>
                        ...
                        <>
                            <div
                                ...
                                <span>This patient has invited you to access their data.</span>
                            </div>
                            <div className="w-full flex flex-col sm:flex-row-reverse gap-3 justify-center">
    ...
    async function sendRequest() {
        ...
        await axiosInstance.post("/doctor/requestPatient", {
            patientUid: data.user.uid
    ...
    async function sendDeletionRequest() {
        ...
        await axiosInstance.post("/doctor/removePatient", {
            patientUid: data.user.uid
        }).catch(function (error) {
            setError(error.response.data)
        })
    ...
    async function sendCancelInvitation() {
        ...
        await axiosInstance.post("/doctor/cancelPatientRequest", {
            patientUid: data.user.uid
        }).catch(function (error) {
            setError(error.response.data)
        })
    ...
    async function refreshRelationshipStatus() {
        setLoading(true)
        const res = await axiosInstance.get(`/doctor/getPatientRelationship?patientUid=${data.user.uid}`)
                                ...
                                <Dialog.Description as="div" className="flex flex-col items-center">
                                    ...
                                    <div className="my-3">
                                        {(!data.user.firstName && !data.user.lastName) ?
                                            ...
                                            :
                                            <h2 className="text-2xl font-semibold text-gray-800">{`${data.user.firstName} ${data.user.lastName}`}</h2>

                                        }
                                    </div>

                                    <div>
                                        <h3 className="text-lg font-medium text-gray-600">{data.user.email}</h3>
                                    </div>
...
export default DoctorViewPatientPopup;